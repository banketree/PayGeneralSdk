...
public class WeiXinUtil {
	...
	public static byte[] httpGet(final String url) {
		if (url == null || url.length() == 0) {
			TLog.e(TAG, "httpGet, url is null");
			return null;
		}
		...
		try {
			...
			if (resp.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				TLog.e(TAG, "httpGet fail, status code = "
			...
			TLog.e(TAG, "httpGet exception, e = " + e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	...
	public static byte[] httpPost(String url, String entity) {
		if (url == null || url.length() == 0) {
			TLog.e(TAG, "httpPost, url is null");
			return null;
		}
		...
		try {
			...
			TLog.e(TAG, "httpPost exception, e = " + e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	...
	public static byte[] readFromFile(String fileName, int offset, int len) {
		...
		if (!file.exists()) {
			TLog.i(TAG, "readFromFile: file not found");
			return null;
		}
		...
		TLog.d(TAG, "readFromFile : offset = " + offset + " len = " + len
				+ " offset + len = " + (offset + len));

		if (offset < 0) {
			TLog.e(TAG, "readFromFile invalid offset:" + offset);
			return null;
		}
		if (len <= 0) {
			TLog.e(TAG, "readFromFile invalid len:" + len);
			return null;
		}
		if (offset + len > (int) file.length()) {
			TLog.e(TAG, "readFromFile invalid file len:" + file.length());
			return null;
		}
		...
		try {
			...
			TLog.e(TAG, "readFromFile : errMsg = " + e.getMessage());
			e.printStackTrace();
		}
		return b;
	}
	...
	public static Bitmap extractThumbNail(final String path, final int height,
		...
		try {
			...
			TLog.d(TAG, "extractThumbNail: round=" + width + "x" + height
					+ ", crop=" + crop);
			final double beY = options.outHeight * 1.0 / height;
			final double beX = options.outWidth * 1.0 / width;
			TLog.d(TAG, "extractThumbNail: extract beX = " + beX + ", beY = "
					+ beY);
			...
			TLog.i(TAG, "bitmap required size=" + newWidth + "x" + newHeight
					+ ", orig=" + options.outWidth + "x" + options.outHeight
					+ ", sample=" + options.inSampleSize);
			Bitmap bm = BitmapFactory.decodeFile(path, options);
			if (bm == null) {
				TLog.e(TAG, "bitmap decode failed");
				return null;
			}
			...
			TLog.e(TAG, "decode bitmap failed: " + e.getMessage());
			options = null;
		}
